...
abstract class FileAdapter implements FFile {
    ...
    public Drawable getIcon() {
        int res;
        if(isDir()) {
            res = R.drawable.ic_action_collection;
        } else {
            switch (getName().substring(getName().lastIndexOf(".") + 1).toLowerCase()) {
                case "php":
                    res = R.drawable.ic_action_php;
                    break;
                case "txt":
                    res = R.drawable.ic_action_txt;
                    break;
                case "png":
                case "jpg":
                case "jpeg":
                case "gif":
                    res = R.drawable.ic_action_picture;
                    break;
                default:
                    res = R.drawable.ic_action_unknown;
                    break;
            }
        }
